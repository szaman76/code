...
#debugger
    ...
    words.each_with_index do |word,idx|
        hash.each do |key,value|
            if key.call(word)
                words[idx] = value.call(words[idx])
            end
        end
    end
    words.join(" ")
...
# The hash contains procs as both keys and values. The method should return a new sentence where each word of the 
# input sentence is changed by a value proc if the original word returns true when passed into the key proc. 
